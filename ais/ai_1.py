...
def def_bord(list_AI,list_Human):
    m = [[0 for x in range(CHESS_LEN)] for y in range(CHESS_LEN)]
...
def def_turn(list_AI,list_Human):
    m = 1
    if len(list_AI) > len(list_Human):
        m = 1
    elif len(list_AI) < len(list_Human):
        m = 2
    else:
        m = 2
    return m
